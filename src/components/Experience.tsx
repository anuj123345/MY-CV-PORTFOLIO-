        ...
        <div className="md:w-2/3 flex flex-col gap-24 md:gap-32 pb-24 md:pb-32 pt-8">
          ...
          <FadeInContainer>
            <motion.div variants={itemVariants} className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
              <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/10"><Code2 className="text-blue-400 w-5 h-5 md:w-6 md:h-6" /></div>
              <h3 className="text-2xl md:text-3xl font-medium">Core Technical Skills</h3>
            </motion.div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              {SKILLS.map((skill, i) => (
                <motion.div variants={itemVariants} key={i} className="group p-5 md:p-6 rounded-[24px] bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] transition-colors hover:border-white/20">
                  <h4 className="text-gray-400 text-xs md:text-sm mb-3 md:mb-4 font-mono uppercase tracking-wider">{skill.category}</h4>
                  <div className="flex flex-wrap gap-2">
                    {skill.items.map((item, j) => (
                      <span key={j} className="px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs md:text-sm text-gray-200 hover:bg-white/10 transition-colors cursor-default">
                        {item}
                      </span>
                    ))}
                  </div>
            ...
            <motion.div variants={itemVariants} className="relative border-l border-white/10 pl-6 md:pl-12 flex flex-col gap-12 md:gap-16">
              ...
              {EXPERIENCE.map((exp, i) => (
                <div key={i} className="relative group transition-all duration-300 hover:translate-x-2">
                  ...
                  <h5 className="text-base md:text-lg text-blue-400 mb-4 md:mb-6">{exp.company}</h5>
                  <ul className="flex flex-col gap-3 md:gap-4">
                    {exp.points.map((point, j) => (
                      <li key={j} className="text-sm md:text-base text-gray-300 leading-relaxed flex gap-2 md:gap-3">
                        <span className="text-white/20 mt-1 md:mt-1.5 shrink-0">▹</span>
          ...
          <FadeInContainer>
            <motion.div variants={itemVariants} className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
              <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/10"><Cpu className="text-emerald-400 w-5 h-5 md:w-6 md:h-6" /></div>
              <h3 className="text-2xl md:text-3xl font-medium">AI Agent Engineering</h3>
            </motion.div>
            <motion.div variants={itemVariants} className="p-6 md:p-8 rounded-[32px] bg-gradient-to-br from-white/[0.05] to-transparent border border-white/10 hover:border-emerald-500/30 transition-colors duration-500 group">
              <ul className="flex flex-col gap-4 md:gap-5">
                {AI_EXPERIENCE.map((point, i) => (
                  <motion.li variants={itemVariants} key={i} className="text-sm md:text-base text-gray-300 leading-relaxed flex gap-3 md:gap-4">
                    <span className="text-emerald-400/50 mt-1 shadow-[0_0_10px_rgba(52,211,153,0.3)] h-1.5 w-1.5 md:h-2 md:w-2 rounded-full shrink-0 group-hover:bg-emerald-400 transition-colors duration-300" />
          ...
          <FadeInContainer>
            <motion.div variants={itemVariants} className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
              <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/10"><Layers className="text-orange-400 w-5 h-5 md:w-6 md:h-6" /></div>
              <h3 className="text-2xl md:text-3xl font-medium">Data & AI Projects</h3>
            </motion.div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              {PROJECTS.map((proj, i) => (
          ...
          <FadeInContainer>
            <motion.div variants={itemVariants} className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8">
              <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/10"><GraduationCap className="text-pink-400 w-5 h-5 md:w-6 md:h-6" /></div>
              <h3 className="text-2xl md:text-3xl font-medium">Education & Training</h3>
            </motion.div>
            <div className="flex flex-col md:flex-row gap-4 md:gap-8">
              <motion.div variants={itemVariants} className="flex-1 p-6 md:p-8 rounded-[32px] bg-white/[0.02] border border-white/5 hover:border-pink-500/30 transition-colors">
                <h4 className="text-gray-400 text-xs md:text-sm mb-4 md:mb-6 font-mono uppercase tracking-wider">Academics</h4>
                {EDUCATION.map((edu, i) => (
                  <div key={i}>
                    <h5 className="text-base md:text-lg text-white font-medium mb-1">{edu.degree}</h5>
                    <p className="text-gray-400 text-xs md:text-sm mb-3">{edu.school} • {edu.year}</p>
                  </div>
                ))}
              </motion.div>
              <motion.div variants={itemVariants} className="flex-1 p-6 md:p-8 rounded-[32px] bg-white/[0.02] border border-white/5 hover:border-pink-500/30 transition-colors">
                <h4 className="text-gray-400 text-xs md:text-sm mb-4 md:mb-6 font-mono uppercase tracking-wider">Certifications</h4>
                <ul className="flex flex-col gap-2 md:gap-3">
                  {CERTS.map((cert, i) => (
                    <li key={i} className="text-gray-300 text-xs md:text-sm flex gap-2 md:gap-3 items-center hover:text-white transition-colors">
